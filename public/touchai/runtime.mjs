...
// packages/touch-runtime/dist/rules.js
...
function gestureEqualsExact(a, b) {
  ...
  switch (a.kind) {
    case "tap":
      return b.kind === "tap" && a.pointerCount === b.pointerCount && approxEqual(a.center.x, b.center.x) && approxEqual(a.center.y, b.center.y);
    case "long_press":
      return b.kind === "long_press" && a.pointerCount === b.pointerCount && approxEqual(a.center.x, b.center.x) && approxEqual(a.center.y, b.center.y) && approxEqual(a.durationMs, b.durationMs, 1);
    case "swipe":
...
function matchPattern(pattern, gesture) {
  if (pattern.kind !== void 0 && pattern.kind !== gesture.kind)
    return false;
  if (pattern.pointerCount !== void 0 && pattern.pointerCount !== gesture.pointerCount) {
    return false;
  }
...
function evaluateProgram(program, gesture) {
  for (const rule of program.rules) {
    ...
    if (when.gesture && gestureEqualsExact(when.gesture, gesture))
      return rule;
    if (when.match && matchPattern(when.match, gesture))
      return rule;
  }
  return void 0;
}

// packages/touch-runtime/dist/haptics-materialize.js
...
// packages/touch-adapter-web/dist/pointer.js
function clamp01(n) {
...
// packages/touch-adapter-web/dist/haptics.js
var WebHapticProgramPlayer = class {
  constructor(navigatorImpl = typeof navigator !== "undefined" ? navigator : null) {
    __publicField(this, "navigatorRef");
  ...
  buildPattern(program) {
    ...
    for (let i = 0; i < program.pulses.length; i++) {
      ...
      const effective = Math.max(1, Math.round(p.durationMs * (0.5 + 0.5 * clamp012(p.intensity))));
      if (i === 0) {
        if (gapMs > 0)
          pattern.push(gapMs, 0);
      } else {
        pattern.push(gapMs);
      }
  ...
  play(program) {
    if (!this.isSupported() || !this.navigatorRef?.vibrate)
      return false;
...
// packages/touch-spec/dist/touch-input.js
var TouchSampleSchema = external_exports.object({
  tMs: external_exports.number().nonnegative(),
  phase: external_exports.enum(["began", "moved", "ended", "cancelled"]),
  ...
  pressure: external_exports.number().min(0).max(1).optional()
});
var GestureTokenSchema = external_exports.discriminatedUnion("kind", [
  external_exports.object({
    kind: external_exports.literal("tap"),
  ...
  external_exports.object({
    kind: external_exports.literal("long_press"),
    center: external_exports.object({ x: external_exports.number(), y: external_exports.number() }),
    durationMs: external_exports.number().positive(),
    pointerCount: external_exports.number().int().min(1).max(10)
  }),
  external_exports.object({
    kind: external_exports.literal("swipe"),
...
// packages/touch-spec/dist/haptic-output.js
var HapticPulseSchema = external_exports.object({
  /** Delay from previous pulse start (or program start for first). */
  delayMs: external_exports.number().nonnegative(),
  /** 0–1 abstract intensity */
  intensity: external_exports.number().min(0).max(1),
  /** 0–1 “sharpness” / transient vs sustained */
  sharpness: external_exports.number().min(0).max(1),
  durationMs: external_exports.number().positive()
});
var HapticProgramSchema = external_exports.object({
  id: external_exports.string().min(1),
  pulses: external_exports.array(HapticPulseSchema).min(1)
});
var HapticSemanticSchema = external_exports.discriminatedUnion("kind", [
  external_exports.object({ kind: external_exports.literal("ack_light") }),
  external_exports.object({ kind: external_exports.literal("ack_medium") }),
  external_exports.object({ kind: external_exports.literal("ack_heavy") }),
  external_exports.object({ kind: external_exports.literal("confirm_success") }),
  external_exports.object({ kind: external_exports.literal("confirm_warning") }),
  external_exports.object({ kind: external_exports.literal("confirm_error") }),
  external_exports.object({ kind: external_exports.literal("understood") }),
  external_exports.object({ kind: external_exports.literal("clarify") }),
  external_exports.object({ kind: external_exports.literal("custom"), program: HapticProgramSchema })
]);

// packages/touch-spec/dist/intent.js
var TouchIntentSchema = external_exports.object({
...
var TouchRuleMatchPatternSchema = external_exports.object({
  ...
  region: external_exports.object({
    ...
    y0: external_exports.number().min(0).max(1),
    x1: external_exports.number().min(0).max(1),
    y1: external_exports.number().min(0).max(1)
  }).optional(),
...
}).refine((w) => w.gesture !== void 0 || w.match !== void 0, {
  message: "when must include gesture and/or match"
});
var TouchRuleActionSchema = external_exports.object({
  intent: TouchIntentSchema.optional(),
  haptic: HapticSemanticSchema.optional()
});
var TouchRuleSchema = external_exports.object({
  id: external_exports.string().min(1),
  when: TouchRuleConditionSchema,
  then: TouchRuleActionSchema
});
var TouchProgramMetaSchema = external_exports.object({
  name: external_exports.string().optional(),
  description: external_exports.string().optional(),
  specVersion: external_exports.string().optional()
});
var TouchProgramSchema = external_exports.object({
  _meta: TouchProgramMetaSchema.optional(),
  rules: external_exports.array(TouchRuleSchema)
});

// packages/touch-spec/dist/envelope.js
var TouchEventEnvelopeSchema = external_exports.object({
  specVersion: external_exports.literal(TOUCHLANG_SPEC_VERSION),
  sessionId: external_exports.string().min(1),
  deviceProfile: external_exports.object({
...
// packages/touch-dataset/dist/jsonl-core.js
function envelopeToJsonlLine(envelope) {
  return `${JSON.stringify(envelope)}
`;
}

// packages/touch-dataset/dist/envelope-client.js
...
function makeEnvelope(input) {
  ...
  if (input.gesture)
    env.gesture = input.gesture;
  if (input.intent)
    env.intent = input.intent;
  if (input.haptic)
    env.haptic = input.haptic;
  return env;
}
function detectDeviceProfile() {
  ...
  const maxPointers = typeof nav.maxTouchPoints === "number" && nav.maxTouchPoints > 0 ? nav.maxTouchPoints : 1;
  return {
    vendor: nav.vendor || void 0,
    model: ua.slice(0, 120) || void 0,
    os: typeof nav.platform === "string" ? nav.platform : void 0,
    maxPointers
  };
}
export {
